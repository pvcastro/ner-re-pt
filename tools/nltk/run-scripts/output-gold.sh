...
do
	...
	do
	 	...
	 	do
	 		...
	 		~/anaconda2/bin/python "../src/out-conll-gold.py" $FOLDER/t-$level-test-doc.txt $OUT/out-$level-gold.txt 'ISO-8859-1'
	 	done

	 	printf "\n** experiences **\n"
	 	FOLDER=../outputs/repeat-$r/joined
	 	~/anaconda2/bin/python "../src/out-conll-gold.py" $FOLDER/t-$level-test-doc.txt $FOLDER/out-$level-gold.txt 'ISO-8859-1'
	done
